export enum externalLink {
    github = 'https://github.com/ThengaraiMaharajan',
    linkedIn = 'https://www.linkedin.com/in/thengarai-maharajan/',
    stackBlitz = 'https://stackblitz.com/@ThengaraiMaharajan',
    codePen = 'https://codepen.io/Tmaharajan',

    changingConstants = 'https://www.changingconstants.io/',
    swiftAnt = 'https://www.swiftant.com/',
    akiInsure = 'https://www.akinsure.com/',
    iam = 'https://www.insuranceassociationmw.com/',
    jerusalem = 'https://www.jerusalemengg.ac.in/',

    edgeDet = 'https://thengaraimaharajandev.github.io/Image-Edge-Detection/',

    fccRWD  = 'https://www.freecodecamp.org/certification/Thengarai_Maharajan/responsive-web-design',
    fccJS = 'https://www.freecodecamp.org/certification/Thengarai_Maharajan/javascript-algorithms-and-data-structures-v8'
}